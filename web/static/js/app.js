...
const state = {
  chatHistory: [],
  activePollers: {},   // key -> intervalId  (one poller per training job)
};
...
// ── Real progress poller ───────────────────────────────────────────
// Polls /api/train_progress/:model/:dataset every 2 s.
// Derives the fill % from actual epoch/total_epochs returned by server.
// Stops only when done===true, then fires onComplete.
function startProgressPoller(model, dataset, fillEl, textEl, onComplete) {
  const key = `${model}_${dataset}`;

  // Clear any existing poller for this key
  if (state.activePollers[key]) {
    clearInterval(state.activePollers[key]);
    delete state.activePollers[key];
  }

  const id = setInterval(async () => {
    try {
      const prog = await api(`/api/train_progress/${model}/${dataset}`);

      if (prog.error && !prog.done) return;   // transient fetch error, keep polling

      const epoch      = prog.epoch       || 0;
      const total      = prog.total_epochs || 1;
      const pct        = total > 0 ? Math.round((epoch / total) * 100) : 0;
      const loss       = prog.loss     != null ? prog.loss.toFixed(4)     : '—';
      const val_loss   = prog.val_loss != null ? prog.val_loss.toFixed(4) : '—';

      // Update bar using REAL percentage (0–100, reaches 100 only when done)
      fillEl.style.width = pct + '%';
      textEl.textContent =
        `Epoch ${epoch}/${total}  |  loss: ${loss}  |  val: ${val_loss}  (${pct}%)`;

      if (prog.error) {
        clearInterval(id);
        delete state.activePollers[key];
        textEl.textContent = `Error: ${prog.error}`;
        if (onComplete) onComplete(false, prog.error);
        return;
      }

      if (prog.done) {
        // Server confirmed training finished — push to 100 %
        fillEl.style.width = '100%';
        textEl.textContent =
          `✓ Complete! ${total} epochs  |  best val: ${val_loss}`;
        clearInterval(id);
        delete state.activePollers[key];
...
async function loadDashboard() {
  ...
  for (const [key, [valId, badgeId]] of Object.entries(mapping)) {
    const d = status[key];
    if (!d) continue;
    document.getElementById(valId).textContent =
      d.best_val != null ? d.best_val.toFixed(4) : '—';
    const badge = document.getElementById(badgeId);
    if (d.trained && !d.training) {
...
async function startTraining() {
  ...
  fill.style.width = '0%';
  txt.textContent  = 'Starting training...';

  startProgressPoller(model, dataset, fill, txt, (success, errMsg) => {
    if (success) {
      showStatus('trainStatus', '✅ Training finished successfully!', 'success');
      setTimeout(() => prog.classList.add('hidden'), 5000);
      loadDashboard();
    } else {
      showStatus('trainStatus', `❌ Training error: ${errMsg}`, 'error');
    }
  });
}

// Train all models sequentially — each waits for the previous to finish
async function trainAllModels() {
  ...
  for (const ds of datasets) {
    for (const mt of models) {
      ...
      item.id = `q_${mt}_${ds}`;
      item.innerHTML = `
        <div class="queue-dot pending" id="dot_${mt}_${ds}"></div>
        <span>${mt} on ${ds}</span>
        <span id="qprog_${mt}_${ds}">Queued</span>`;
      queueEl.appendChild(item);
    }
  }

  for (const { mt, ds } of jobs) {
    const key = `${mt}_${ds}`;
    document.getElementById(`dot_${key}`).className   = 'queue-dot running';
    document.getElementById(`qprog_${key}`).textContent = 'Starting…';

    // Launch training
    const res = await api('/api/train', 'POST', { model_type: mt, dataset: ds });
    if (res.error) {
      document.getElementById(`dot_${key}`).className    = 'queue-dot';
      document.getElementById(`dot_${key}`).style.background = '#ff6b9d';
      document.getElementById(`qprog_${key}`).textContent = `Error: ${res.error}`;
      continue;
    }

    // Wait for this job to finish via real progress endpoint
    await new Promise(resolve => {
      const id = setInterval(async () => {
        try {
          const prog = await api(`/api/train_progress/${mt}/${ds}`);
          const epoch = prog.epoch || 0;
          const total = prog.total_epochs || 1;
          const pct   = total > 0 ? Math.round((epoch / total) * 100) : 0;
          document.getElementById(`qprog_${key}`).textContent =
            `${pct}%  (${epoch}/${total})`;

          if (prog.done || (!prog.error && epoch >= total && total > 0)) {
            clearInterval(id);
            document.getElementById(`dot_${key}`).className      = 'queue-dot done';
            document.getElementById(`qprog_${key}`).textContent  = '✓ Done';
            resolve();
          }
          if (prog.error) {
            clearInterval(id);
            document.getElementById(`qprog_${key}`).textContent = `Error`;
            resolve();
          }
...
async function loadLatentSpace() {
  ...
  const traces = Object.entries(grouped).map(([label, pts]) => ({
    x: pts.x, y: pts.y,
  ...
  Plotly.newPlot('latentPlot', traces, {
    paper_bgcolor: '#1a1a25', plot_bgcolor: '#16161f',
    font: { color: '#e8e8f0' },
  ...
  document.getElementById('silhouetteDisplay').innerHTML = `
...
async function loadComparison() {
  ...
  if (!data.comparison || Object.keys(data.comparison).length === 0) {
    wrap.innerHTML = '<p style="color:var(--text2);padding:20px">Train models first.</p>';
    return;
  }
...
async function runAIAnalysis() {
  ...
  outputEl.textContent = '⟳ Analyzing results with Llama 3.3 70B...';
  ...
  outputEl.textContent = data.error ? `Error: ${data.error}` : data.analysis;
}
...
async function sendChat() {
  ...
  const data  = await api('/api/ai_chat', 'POST', { messages: state.chatHistory });
